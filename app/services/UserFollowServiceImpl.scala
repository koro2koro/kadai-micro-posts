...
class UserFollowServiceImpl extends UserFollowService {
  ...
  override def findFollowersByUserId(pagination: Pagination, userId: Long)(implicit dbSession: DBSession = AutoSession): Try[PagedItems[User]] = {
    countByFollowId(userId).map { size =>
      PagedItems(pagination, size,
        UserFollow.allAssociations.findAllByWithLimitOffset(
